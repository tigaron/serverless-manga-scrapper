...
function _crawler() {
  _crawler = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(urlString) {
        ...
        switch (_context.prev = _context.next) {
          ...
          case 18:
            page.on("request", function (request) {
              if (request.resourceType() !== "document") request.abort();else request["continue"]();
            });
            _context.next = 21;
            return page["goto"](urlString, {
...
function parseMangaList($, mangaProvider) {
  var MangaList = new Set();
  $("a.series", "div.soralist").each(function (index, element) {
    var MangaTitle = $(element).text().trim();
    var MangaUrl = $(element).attr("href");
    var MangaSlug = MangaUrl.split("/").slice(-2).shift().replace(/[\d]*[-]?/, "");
    var timestamp = new Date().toUTCString();
    var Manga = new Map([["EntryId", "manga_".concat(mangaProvider)], ["EntrySlug", MangaSlug], ["MangaTitle", MangaTitle], ["MangaUrl", MangaUrl], ["ScrapeDate", timestamp]]);
    MangaList.add(Manga);
  });
  return MangaList;
}
...
function parseManga($, mangaProvider) {
  var MangaTitle = $("h1.entry-title").text().trim();
  var MangaSynopsis = $("p", "div.entry-content").text();
  if (!MangaSynopsis) MangaSynopsis = $("div.entry-content").contents().text();
  var MangaCover = $("img", "div.thumb").attr("src");
  var MangaShortUrl = $("link[rel='shortlink']").attr("href");
  var MangaCanonicalUrl = $("link[rel='canonical']").attr("href");
  var MangaSlug = MangaCanonicalUrl.split("/").slice(-2).shift().replace(/[\d]*[-]?/, "");
  var timestamp = new Date().toUTCString();
  var Manga = new Map([["EntryId", "manga_".concat(mangaProvider)], ["EntrySlug", MangaSlug], ["MangaTitle", MangaTitle], ["MangaSynopsis", MangaSynopsis], ["MangaCover", MangaCover], ["MangaShortUrl", MangaShortUrl], ["MangaCanonicalUrl", MangaCanonicalUrl], ["ScrapeDate", timestamp]]);
  return Manga;
}
...
function parseChapterList($, mangaProvider) {
  ...
  $("a", "div.eplister").each(function (index, element) {
    ...
    var ChapterOrder = $(element).parents("li").data("num");
...
function parseChapter($, mangaProvider) {
  ...
  if (mangaProvider === "realm") {
    var realmContent = $("div#readerarea").contents().text();
    $("img[class*='wp-image']", realmContent).each(function (index, element) {
      var img = $(element).attr("src");
      ChapterContent.add(img);
    });
  } else {
    $("img[class*='wp-image']", "div#readerarea").each(function (index, element) {
      var img = $(element).attr("src");
      ChapterContent.add(img);
    });
  }
...
function scraper(_x2, _x3, _x4) {
  return _scraper.apply(this, arguments);
}

function _scraper() {
  _scraper = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(urlString, requestType, mangaProvider) {
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            _context2.prev = 0;
            _context2.next = 3;
            return crawler(urlString);

          case 3:
            htmlString = _context2.sent;
            $ = loadHTML(htmlString);
            _context2.t0 = requestType;
            _context2.next = _context2.t0 === "MangaList" ? 8 : _context2.t0 === "Manga" ? 10 : _context2.t0 === "ChapterList" ? 12 : _context2.t0 === "Chapter" ? 14 : 16;
            break;
