...
function parseManga ($, provider) {
  ...
  try {
    ...
    logger.error(`parseManga failed: `, error.message);
    Manga.clear();
  }
  logger.debug(`parseManga result:`, Manga);
  return Manga;
}
  ...
  try {
    $('a', 'div.eplister').each((index, element) => {
      ...
      const Chapter = new Map([
        ...
        ['ChapterDate', $('span.chapterdate', element).text().trim()],
        ['ScrapeDate', new Date().toUTCString()],
      ]);
